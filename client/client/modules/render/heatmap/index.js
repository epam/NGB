...
import HeatmapColorScheme, {
    ColorFormats,
    colorSchemes as HeatmapColorSchemes,
} from './color-scheme';
import HeatmapData, {HeatmapDataType} from './heatmap-data';
...
class HeatmapView extends HeatmapEventDispatcher {
    ...
    initialize(options = {}) {
        ...
        if (initialized !== this.initialized) {
            this.metadataLoaded(this.heatmapData);
        }
    ...
    updateDisplayOptions(options = {}) {
        ...
        if (pixiRenderer && domElement) {
            const invalidateAll = pixiRenderer !== this.pixiRenderer || domElement !== this.domElement;
            if (!this.container) {
                /**
                 * Root container
                 * @type {PIXI.Container}
