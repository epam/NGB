...
export default class WIGRenderer extends CachedTrackRenderer {
    ...
    _changeWig(viewport, wig, coordinateSystem) {
        ...
        {
            const { block, line } = this._renderItems(
                wig.thresholdItems.aboveBaseAxis,
                this._config.wig.thresholdColor,
                this._config.wig.lineThresholdColor,
                viewport,
                wig,
                coordinateSystem,
            );
        ...
        {
            const { block, line } = this._renderItems(
                wig.thresholdItems.belowBaseAxis,
                this._config.wig.thresholdColor,
                this._config.wig.lineThresholdColor,
                viewport,
                wig,
    ...
    hoverItem(item, viewport, wig, coordinateSystem = {}) {
        ...
        if (item) {
            ...
            const isDetailed =
                wig.isDetailed ||
                viewport.factor > this._config.wig.detailedStyleStartingAtPixelsPerBP;

            block.beginFill(0x000000, viewport.factor > 2 ? 0.125 : VIEWPORT_PADDING);
            const lineThickness = 1;
            line.lineStyle(lineThickness, 0x000000, viewport.factor > 2 ? VIEWPORT_PADDING : 1);

            const padding = isDetailed ? VIEWPORT_PADDING : 0;
            const startX1 = Math.round(
                viewport.project.brushBP2pixel(item.startIndex - VIEWPORT_PADDING) + padding,
            );
            let startX2 = Math.round(
                viewport.project.brushBP2pixel(item.endIndex + VIEWPORT_PADDING) - padding,
            );
            if (startX1 === startX2) {
                startX2++;
            }
            ...
            line.moveTo(
                startX1,
                this._getYValue(item.dataValue, coordinateSystem) - lineThickness / 2,
            );
            line.lineTo(
                startX2,
                this._getYValue(item.dataValue, coordinateSystem) - lineThickness / 2,
    ...
    _getPercentValue(value, coordinateSystem) {
        return (
            (value - coordinateSystem.minimum) /
            (coordinateSystem.maximum - coordinateSystem.minimum)
        );
    }
    ...
    onMove(viewport, cursor, data) {
        if (data && data.items && data.thresholdItems) {
            return (
                this._checkItems(viewport, cursor, data.items.aboveBaseAxis, {
                    above: true,
                    threshold: false,
                }) ||
