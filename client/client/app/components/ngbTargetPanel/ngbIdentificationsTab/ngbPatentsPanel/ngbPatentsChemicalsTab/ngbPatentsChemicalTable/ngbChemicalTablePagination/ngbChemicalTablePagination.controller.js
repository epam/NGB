...
export default class ngbChemicalTablePaginationController {
    ...
    constructor($scope, $timeout, dispatcher, ngbPatentsChemicalsTabService, ) {
        ...
        dispatcher.on('target:identification:patents:drug:results:updated', this.refresh.bind(this));
        $scope.$on('$destroy', () => {
            dispatcher.removeListener('target:identification:patents:drug:results:updated', this.refresh.bind(this));
        });
    }
